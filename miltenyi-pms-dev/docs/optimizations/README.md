...
| [07](./07-project-reviews-migration.md) | ProjectReviews migration: 5 role-gated queries + cache-warming probe pattern | _pending_ | 5 useQuery calls (4 role-gated, 1 universal probe); `hasSecondaryWork` derived from cached data instead of a separate state flag |
