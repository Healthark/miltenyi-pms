  ...
  return (
    <section className="space-y-2">
      {/* Amber callout — the section IS the alert. Same visual language
          as MentorCoverageCard's row-level orphan chip so the user
          reads "orphan" the same way across cards. */}
      <div className="rounded-lg border border-amber-300 dark:border-amber-500/40 bg-amber-50/60 dark:bg-amber-500/10 px-3 py-2.5">
        <div className="flex items-center gap-1.5 mb-1.5">
          ...
          <span className="ml-auto inline-flex items-center rounded-full bg-amber-200/80 dark:bg-amber-500/20 px-2 py-0.5 text-[11px] font-semibold text-amber-900 dark:text-amber-200 tabular-nums">
            {count} {word}
          </span>
        </div>
        <p className="text-[11px] text-amber-800/90 dark:text-amber-300/90 mb-2">
          Their previous PM was deactivated or role-changed. Assign a
          new PM to unfreeze pending project reviews.
        </p>
        <div className="rounded-md border border-amber-200 dark:border-amber-700/40 bg-white/70 dark:bg-slate-900/40 max-h-40 overflow-y-auto divide-y divide-amber-200/70 dark:divide-amber-700/30">
          {orphaned.map((p) => (
            <Link
              key={p.project_id}
              to={`/admin?tab=projects&edit=${p.project_id}`}
              className="flex items-center justify-between gap-3 px-3 py-1.5 hover:bg-amber-50/50 dark:hover:bg-amber-500/10"
            >
              ...
              <p className="text-[11px] text-amber-700 dark:text-amber-300 whitespace-nowrap shrink-0">
...
function SkeletonBody() {
  return (
    <div className="space-y-2 animate-pulse">
      <div className="rounded-lg border border-amber-200 dark:border-amber-700/40 bg-amber-50/40 dark:bg-amber-500/10 p-3 space-y-2">
        <div className="flex items-center gap-2">
          <div className="h-3 w-32 rounded bg-amber-100 dark:bg-amber-500/20" />
          <div className="ml-auto h-4 w-20 rounded-full bg-amber-100 dark:bg-amber-500/20" />
        </div>
        <div className="rounded-md border border-amber-200 dark:border-amber-700/30 bg-white/60 dark:bg-slate-900/40 divide-y divide-amber-100 dark:divide-amber-700/20">
          {[0, 1, 2].map((row) => (
            <div
              key={row}
              className="flex items-center justify-between gap-3 px-3 py-2"
            >
              <div className="h-3 w-48 rounded bg-amber-100 dark:bg-amber-500/20" />
              <div className="h-3 w-20 rounded bg-amber-100 dark:bg-amber-500/20" />
            </div>
          ))}
        </div>
      </div>
